...
# 프루닝 비율 딕셔너리 정의
pruning_percents = {
    'features.0.weight': 5,   # Conv1
    'features.2.weight': 5,   # Conv2
    'features.5.weight': 5,   # Conv3
    'features.7.weight': 5,   # Conv4
    'features.10.weight': 10, # Conv5
    'features.12.weight': 10, # Conv6
    'features.14.weight': 10, # Conv7
    'features.17.weight': 20, # Conv8
    'features.19.weight': 20, # Conv9
    'features.21.weight': 20, # Conv10
    'features.24.weight': 30, # Conv11
    'features.26.weight': 30, # Conv12
    'features.28.weight': 30, # Conv13
}

# 로깅 설정
def setup_logging():
    current_time = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    log_directory = "VGG16/log"
...
def prune_model_filters_by_importance(model, train_loader, test_loader, target_layers, pruning_percents, algorithm='kruskal'):
    model = model.to(device)
...
def main(args):
    ...
    # 저장된 모델 경로 설정
    save_directory = 'saved_models/VGG16'
